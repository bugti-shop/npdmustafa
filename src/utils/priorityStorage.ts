...
export const savePriorities = async (priorities: CustomPriority[]): Promise<void> => {
  await setSetting(STORAGE_KEY, priorities);
  // Dispatch event to notify components
  window.dispatchEvent(new CustomEvent('prioritiesChanged', { detail: priorities }));
};
...
export const updatePriority = async (id: string, updates: Partial<Omit<CustomPriority, 'id' | 'isDefault'>>): Promise<void> => {
  const priorities = await getPriorities();
